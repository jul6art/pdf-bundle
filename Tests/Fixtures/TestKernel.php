...
{
    /**
     * @param array<string, mixed> $bundleConfig configuration for the "pdf" extension
     * @param bool                 $withTwig     registers TwigBundle — needed only by the
     *                                           scenario proving PdfAssetExtension actually
     *                                           registers when Twig is present
        ...
        $container->addCompilerPass(new class implements CompilerPassInterface {
            ...
            public function process(ContainerBuilder $container): void
            {
                $exposed = [
                    ...
                    PdfArchiverInterface::class,
                    TemplateWhitelistResolver::class,
    ...
    {
        return \sprintf('%s/jul6art-pdf-bundle-tests/%s/%s', sys_get_temp_dir(), $this->uniqueId, $this->environment);
    }

    private function configure(ContainerBuilder $container): void
    {
        $container->loadFromExtension('framework', [
            'secret' => 'jul6art-pdf-bundle-tests',
            'http_method_override' => false,
            'handle_all_throwables' => true,
            'php_errors' => ['log' => true],
            // LocaleSwitcher is bound to the "translator" service id — a real application always
            // has one, so the bundle's own tests need one too.
        ...
        $container->loadFromExtension('pdf', $this->bundleConfig);
    }
}
