...
namespace Jul6Art\PdfBundle\Asset;
 ...
 * remote URLs in production (`isRemoteEnabled = false`, see {@see \Jul6Art\PdfBundle\Renderer\DompdfRenderer})
 * and has no base to resolve a schemeless relative one — so the image simply never loads, silently.
 *
 * ```twig
 * {# a filesystem path, when the renderer is allowed to read the directory #}
 ...
 *
 * ⚠️ Moved here from `jul6art/core-bundle` (`core-bundle` v3.0.0). Configured internally, it stays
 * unused unless a consumer actually calls one of the two functions — a real, observed gap: one
 * consuming project configured `core.pdf.public_dir` and never called either function, so its
 * organization logo never printed on a single PDF despite a comment claiming it did.
 */
final class PdfAssetExtension extends AbstractExtension
{
     ...
     * produce a well-formed URI that the renderer draws as a **white square**, which is worse than
